# =============================================================
# REPORT 1: TIN TỨC TUẦN (Chủ nhật 6:00 ICT)
# =============================================================
...
def fetch_ff_events():
    ...
    for e in root.findall("event"):
        ...
        events.append({"dt":dt,"day_name":dt.strftime("%A"),"date_vi":dt.strftime("%d/%m"),
            "time_ict":ict_time,"impact":impact,"title":e.findtext("title",""),
...
def build_report(events, cfg):
    ...
    next_mon = now_ict + timedelta(days=1); next_fri = now_ict + timedelta(days=5)
    week_str = f"{next_mon.strftime('%d/%m')} - {next_fri.strftime('%d/%m/%Y')}"
    lines = [f"<b>{rc['header']}</b>","━━━━━━━━━━━━━━━━━━━━",
        f"📅 Tuần: {week_str}",f"🕘 Cập nhật: {now_ict.strftime('%d/%m/%Y %H:%M')} ICT","",
        "<b>Lịch sự kiện USD quan trọng tuần tới</b>","<i>(High 🔴 | Medium 🟠 — giờ ICT)</i>"]
    ...
    if not events: lines.append("\n⚠️ Không có sự kiện USD High/Medium tuần này.")
...
def main():
    ...
    if not cfg["reports"]["weekly_news"].get("enabled", True):
        print("[Weekly News] Disabled — bỏ qua"); return
